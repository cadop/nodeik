...
from nodeik.robots.robot import Robot, RobotTocabiSingle
...
def get_robot(filepath):
    ...
    r = RobotTocabiSingle(robot_path=filepath, ee_link_name='L_Wrist2_Link')
...
def run():
    import wandb
    run = wandb.init()
    artifact = run.use_artifact('psh117/node-ik/model-32c65ba9:v133', type='model')
    ...
    custom_q = np.array([0,0,0, 
                -0.38, -0.1, 0.99, 1.56, 1.50, 1.04, 1.30,0.0], dtype=np.float32)
    fk_2 = r.get_forward_kinematics(custom_q)
    ...
    for angle in np.linspace(0,2*np.pi, args.num_samples, endpoint=False):
        ...
        targets.append(target_pose_left)
