...
def run():
    ...
    for angle in np.linspace(0,2*np.pi, args.num_samples, endpoint=False):
        ...
        # target_pose_left = np.array([fk_2[0]+radius*sin(angle), fk_2[1], fk_2[2]+radius*cos(angle)] ,dtype=np.float32)
    ...
    render(f'0_new_tocabi_{x_dist}_{y_dist}_{z_dist}_{radius}_{std}_{args.seed}_for', f'tocabi_for_ref', ik_q)
    ...
    for t in tqdm(range(trials)):
        ...
        if is_path_continuous(ik_q):
            render(f'0_new_tocabi_{x_dist}_{y_dist}_{z_dist}_{radius}_{std}_{args.seed}_for', f'tocabi_for_{t}', ik_q)
    exit()
